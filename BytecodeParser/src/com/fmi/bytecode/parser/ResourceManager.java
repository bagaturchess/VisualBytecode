...
public final class ResourceManager
{
    ...
    */
    ...
    public static void main1(final String[] args) throws Exception {
        ...
        p.load(new FileInputStream("D:/Perforce/InQMy/Bytecode/dev/src/com/sap/engine/library/bytecode/res/res.properties"));
        final ArrayList list = new ArrayList();
        final Enumeration _enum = p.propertyNames();
        ...
        for (int i = 0; i < names.length; ++i) {
            System.out.println("  public static final int " + names[i] + " = " + i + ";");
        }
        System.out.println();
        System.out.println("  private static final String[] PROPERTY_NAMES = {");
        for (int i = 0; i < names.length; ++i) {
    ...
    public static void main(final String[] args) throws Exception {
        final ByteArrayOutputStream out0 = new ByteArrayOutputStream(5000);
        final PrintStream out2 = new PrintStream(out0);
        final File file = new File("D:/Perforce/InQMy/Bytecode/dev/src/" + ResourceManager.class.getName().replace('.', '/') + ".java");
        final BufferedReader in = new BufferedReader(new FileReader(file));
        while (true) {
            final String s = in.readLine();
            if (s == null || s.indexOf("GENERATED:") != -1) {
                break;
            }
            out2.println(s);
        }
        out2.println("  // GENERATED:");
        out2.println();
        in.close();
        final Properties p = new Properties();
        p.load(new FileInputStream("D:/Perforce/InQMy/Bytecode/dev/src/com/sap/engine/library/bytecode/res/res.properties"));
        ...
        Arrays.sort(names);
        for (int i = 0; i < names.length; ++i) {
            out2.println("  public static final int " + names[i] + " = " + i + ";");
        }
        out2.println();
        out2.println("  private static final String[] PROPERTY_NAMES = {");
        for (int i = 0; i < names.length; ++i) {
            out2.println("    \"" + names[i] + "\", // " + i);
        }
        out2.println("  };");
        out2.println("}");
        out2.println();
        out2.close();
        final byte[] data = out0.toByteArray();
        final FileOutputStream out3 = new FileOutputStream(file);
        out3.write(data);
        out3.close();
    }
}